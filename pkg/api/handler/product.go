...
func (cr *ProductHandler) ListAllCategories(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse(500, "failed to fetch the categories", err.Error(), nil))
		return
	}

	c.JSON(http.StatusOK, response.SuccessResponse(200, "Succesfully fetched all categories", categories))
...
func (cr *ProductHandler) FindCategoryByID(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusUnprocessableEntity, response.ErrorResponse(422, "failed to parse the category id", err.Error(), nil))
		return
	}
	category, err := cr.productUseCase.FindCategoryByID(c.Request.Context(), categoryID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse(500, "failed to fetch the category", err.Error(), nil))
		return
	}
	c.JSON(http.StatusOK, response.SuccessResponse(200, "Succesfully fetched the category details", category))
...
func (cr *ProductHandler) UpdateCategory(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse(401, "unable to update the category", err.Error(), nil))
		return
	}
	c.JSON(http.StatusAccepted, response.SuccessResponse(202, "Succesfully updated the category", updatedCategory))
...
func (cr *ProductHandler) DeleteCategory(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse(400, "unable to delete the category, products listed with this category", err.Error(), nil))
		return
	}
	c.JSON(http.StatusAccepted, response.SuccessResponse(202, "Succesfully deleted the category", deletedCategory))
...
func (cr *ProductHandler) FindProductByID(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusUnprocessableEntity, response.ErrorResponse(422, "failed to parse the product id", err.Error(), nil))
		return
	}
	product, err := cr.productUseCase.FindProductByID(c.Request.Context(), productID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse(500, "Unable to find the product", err.Error(), nil))
		return
	}
	c.JSON(http.StatusOK, response.SuccessResponse(200, "Succesfully fetched the product", product))
}
...
func (cr *ProductHandler) UpdateProduct(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse(401, "unable to update the product", err.Error(), nil))
		return
	}
	c.JSON(http.StatusAccepted, response.SuccessResponse(202, "Succesfully updated the product", updatedProduct))
...
func (cr *ProductHandler) DeleteProduct(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusNotFound, response.ErrorResponse(404, "No products are available in this id", err.Error(), nil))
		return
	}

	err = cr.productUseCase.DeleteProduct(c.Request.Context(), productID)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse(400, "unable to delete the products", err.Error(), nil))
		return
	}
	c.JSON(http.StatusAccepted, response.SuccessResponse(202, "Succesfully deleted the product", nil))
}
